...
def Main():
    ...
    while True:
        ...
        print(f"6) Скачать Проект и Опубликовать по IP машины")
        print(f"7) Тор Публикация")
        print(f"8) Зарегестрировать Протокол HTTPS")
        print(f"9) Очистить Консоль")
        print(f"10) Выход из Скрипта")
        ...
        if result=="3":
            ...
            if res==True:
                ...
                if resdel.lower()=="y":
                    ...
                    # Чистка Хоста
                    for hos in li["HostWebs"]:
                        proj_nginx.LinexHost_Del(li["IP"],hos)
                    # Удаление Проекта
        ...
        if result=="6":
            print(f"1) Проект по IP машины один")
            print(f"2) Удалить Проект по IP машины один")
            print(f"3) Вернуться")
            result_proj = app.InputWhile("Номер Выбора: ")
            if result_proj=="1":
                nameproject = app.InputWhile("Имя Проекта: ")
                arhiveproject = app.InputWhile("Имя Файл Архива: ")
                filerunproject = app.InputWhile("Запускаймый Файл: ")
            ...
            if result_proj=="2":
                ...
                if res==True:
                    ...
                    resdel = app.InputWhile("Вы Уверены Удалении Y/N: ")
                    if resdel.lower()=="y":
                        # Резервирование Вернуть Настроек Nginx default
                        dir_reserv="/etc/nginx/backfiles"
                        resdef=proj_nginx.Ngnix_Restore(f"{dir_reserv}/default", "/etc/nginx/sites-available/default")
                        if resdef==True:
                            print("Файл default Вернулся!")
                        else:
                            print("Файл default Ошыбка Нету Резервной Копии!") 
                        # ------------Адреса----------------------
                        sites_available_file = f'/etc/nginx/sites-available/{li["NginxFile"]}'
                        sites_enabled_file= f'/etc/nginx/sites-enabled/{li["NginxFile"]}'
                        systemd_service_file = f'/etc/systemd/system/{li["ServiceFile"]}'
        ...
        if result=="10":
            break
        elif result!="1" and result!="2" and result!="3" and result!="4" and result!="5" and result!="6" and result!="7" and result!="8" and result!="9" and result!="10":
